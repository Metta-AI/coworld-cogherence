...
- **Coherence = margin of dominance.** Coherence moves with the upkeep bill: a tile whose bill goes unpaid while under resistance (more enemy than allied neighbors) loses 1 (neutral at 0) — zero-resistance ground holds; paying a flat 3e regen on top of the bill gains 1 (max 1/turn). The bill is a base of floor(sqrt(tiles owned)) — empire scale taxes every tile — + resistance = 10e × ceil(enemies − allies/2), floored at 0, each allied neighbor offsetting half an enemy (neutral counts for neither). An Align is a tug-of-war where the winner's new coherence = its force minus the runner-up's. Aligns are paid in ENERGY (max 100 each): arriving force = floor(sqrt(energy − distance²)) from the aligner's closest tile, full price charged win or lose — so 100e at distance 0 arrives as force 10 (the coherence cap), and reach is quadratically expensive. Any in-board tile is a legal target.
