...
describe("resolve", () => {
  ...
  it("rejects the WHOLE order set if any order is illegal (the exploit does not happen)", () => {
    ...
    const orders: Record<CogId, Order[]> = {
      A: [ { type: "align", tile: "3,0", energy: 1 }, { type: "exploit", tile: "0,0" } ], // (3,0) not adjacent to A -> illegal
    };
  ...
  it("rejects an unaffordable order set", () => {
    const s = makeState({ tiles: [tile(0, 0, "A", 3)], cogOrder: ["A"], treasuries: { A: T() } });
    const { state, events } = resolve(s, { A: [{ type: "align", tile: "0,0", energy: 5 }] });
    expect(at(state, 0, 0).coherence).toBe(3); // unchanged
  ...
  it("exploit resolves BEFORE align: an exploited tile is neutral/0 when a rival's align lands, so the rival takes the husk", () => {
    const s = makeState({
      tiles: [tile(0, 0, "A", 5, "C", 2), tile(1, 0, "B", 3)], cogOrder: ["A", "B"],
      treasuries: { A: T(), B: T(1, 1, 1, 1) },
    });
    const { state } = resolve(s, {
      A: [{ type: "exploit", tile: "0,0" }],
      B: [{ type: "align", tile: "0,0", energy: 3 }], // B owns adjacent (1,0) -> legal
    });
    expect(at(state, 0, 0)).toMatchObject({ alignment: "B", coherence: 3 });
    expect(tre(state, "A").C).toBe(20); // 2*5*2 windfall
  });

  it("two Cogs contest a neutral tile: higher Align energy wins, coherence = margin", () => {
    const s = makeState({
      tiles: [tile(0, 0, null, 0), tile(1, 0, "A", 4), tile(-1, 0, "B", 4)], cogOrder: ["A", "B"],
      treasuries: { A: T(2, 2, 2, 2), B: T(2, 2, 2, 2) },
    });
    const { state } = resolve(s, {
      A: [{ type: "align", tile: "0,0", energy: 5 }],
      B: [{ type: "align", tile: "0,0", energy: 3 }],
    });
    expect(at(state, 0, 0)).toMatchObject({ alignment: "A", coherence: 2 });
  });
  ...
  it("integration: a siege + an auction in one turn", () => {
    const s = makeState({
      tiles: [tile(0, 0, "A", 4), tile(1, 0, "B", 4)], cogOrder: ["A", "B"],
      treasuries: { A: T(2, 2, 2, 2), B: T(2, 2, 2, 2) },
    });
    const { state, events } = resolve(s, {
      A: [{ type: "align", tile: "1,0", energy: 7 }, { type: "bid", energy: 4 }], // siege B's tile + bid
      B: [{ type: "align", tile: "1,0", energy: 2 }, { type: "bid", energy: 2 }], // reinforce + bid
    });
    expect(at(state, 1, 0)).toMatchObject({ alignment: "A", coherence: 1 }); // A force 7 vs B 4+2=6
    expect(state.cogs.A!.hearts).toBe(1);
    expect(tre(state, "A")).toEqual(T(1, 1, 1, 1)); // spent 7 (siege) + 2 (clearing) = 9 -> burns a set (overshoot)
