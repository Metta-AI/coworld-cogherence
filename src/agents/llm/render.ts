...
import { COHERENCE_MAX, MAX_TURNS, MINT_DIVISOR, REGEN_COST, RESISTANCE_COST, SET_ENERGY, UPKEEP_BASE } from "../../shared/engine/constants";
...
ENERGY. Aligned tiles mint density×coherence/${MINT_DIVISOR} of their mineral each turn (rounded to a whole unit, probabilistically) — a tile at full coherence (${COHERENCE_MAX}) yields DOUBLE its density; weaker tiles yield proportionally less. Minerals convert to energy on demand: a full C+O+Ge+S set = ${SET_ENERGY} energy, a single leftover mineral = 1. So a balanced treasury is far more efficient — trade is survival. Each tile bills upkeep every turn: a flat ${UPKEEP_BASE} energy base PLUS resistance = ${RESISTANCE_COST} energy × ceil(enemy neighbors − allied neighbors / 2), floored at 0 — each ally offsets only HALF an enemy, neutral neighbors count for neither side, and contested ground is a serious money sink. Coherence changes ONLY through this bill: an unpaid tile UNDER resistance (resistance > 0) loses 1 Coherence (at 0 it goes neutral and you lose it) while zero-resistance tiles hold even unpaid; paying ${REGEN_COST} energy ON TOP of a tile's bill regenerates +1 Coherence (max 1/turn, cap ${COHERENCE_MAX}). Bills and regen are paid strongest-tile-first automatically. IMPORTANT: minerals you mint this turn land in your treasury NEXT turn (a one-turn lag), so you can only spend the energy you ALREADY hold; an order set you can't afford is rejected wholesale.

YOUR ACTIONS each turn (via the submit_orders tool):
- align {tile, energy}: commit ENERGY (max 100 per align) to a tile's tug-of-war. The force ARRIVING = floor(sqrt(energy − distance²)), where distance is from your CLOSEST tile (your own tile = 0, adjacent = 1) — so 100e at distance 0 arrives as force 10 (the coherence cap), and reach is quadratically expensive. The FULL energy is charged win or lose; an align whose force fully dissipates is rejected, and so is any set you cannot afford. Capture is a tug-of-war: you take a tile when your arriving force exceeds the incumbent's Coherence (plus any force they commit).
