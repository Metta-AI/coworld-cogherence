...
import { COHERENCE_MAX, MAX_TURNS, SET_ENERGY, UPKEEP_BASE, DRIFT_GAIN_COST } from "../../shared/engine/constants";
...
ENERGY. Aligned tiles mint density×coherence/${COHERENCE_MAX} of their mineral each turn (rounded to a whole unit, probabilistically) — so a tile at full coherence (${COHERENCE_MAX}) yields its density, and weaker tiles yield proportionally less. Minerals convert to energy on demand: a full C+O+Ge+S set = ${SET_ENERGY} energy, a single leftover mineral = 1. So a balanced treasury is far more efficient — trade is survival. Each tile costs ${UPKEEP_BASE} + floor(sqrt(yourTileCount)/3) energy/turn upkeep — bigger empires pay a higher per-tile rate. Rising order is not free either: every tile that GAINS +1 Coherence at Upkeep drains ${DRIFT_GAIN_COST} energy (gains you cannot afford are forfeited, strongest tiles funded first). IMPORTANT: minerals you mint this turn land in your treasury NEXT turn (a one-turn lag), so you can only spend the energy you ALREADY hold; an order set you can't afford is rejected wholesale.
