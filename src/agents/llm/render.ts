...
import { COHERENCE_MAX, MAX_TURNS, SET_ENERGY, UPKEEP_CALM, UPKEEP_CONTESTED } from "../../shared/engine/constants";
...
ENERGY. Aligned tiles mint density×coherence/${COHERENCE_MAX} of their mineral each turn (rounded to a whole unit, probabilistically) — so a tile at full coherence (${COHERENCE_MAX}) yields its density, and weaker tiles yield proportionally less. Minerals convert to energy on demand: a full C+O+Ge+S set = ${SET_ENERGY} energy, a single leftover mineral = 1. So a balanced treasury is far more efficient — trade is survival. Each tile bills upkeep every turn: ${UPKEEP_CONTESTED} energy normally, ${UPKEEP_CALM} if a strict majority of its in-board neighbors share your alignment, PLUS 1 per ENEMY-aligned neighbor (friendly neighbors never add cost; contested borders are expensive). Coherence changes ONLY through this bill: a tile whose bill you cannot pay loses 1 Coherence (at 0 it goes neutral and you lose it); a tile you pay DOUBLE for gains +1 Coherence (max ${COHERENCE_MAX}). Bills are paid strongest-tile-first automatically. IMPORTANT: minerals you mint this turn land in your treasury NEXT turn (a one-turn lag), so you can only spend the energy you ALREADY hold; an order set you can't afford is rejected wholesale.

YOUR ACTIONS each turn (via the submit_orders tool):
- align {tile, coherence}: pour COHERENCE toward your alignment — it is TRANSFERRED out of your other tiles, largest first (they never drop below 1), so expansion literally spends your standing order. Your total Aligns must fit your spare coherence (sum over your tiles of coherence−1) or the whole order set is rejected. Target a tile you own (reinforce) or one adjacent to your territory (expand/capture). Capture is a tug-of-war: you take a tile when your force exceeds the incumbent's Coherence.
