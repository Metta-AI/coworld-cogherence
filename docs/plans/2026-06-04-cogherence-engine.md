...
**Energy (`chargeEnergy(treasury, need)`):** energy is derived on demand, never stored. **Affordable iff `maxEnergy(treasury) >= need`** — affordability is *monotonic*, which the second-price auction requires (you bid `B` but pay a lower clearing price `P`). To cover `need`: burn a whole COGS set while the remaining need `>= SET_ENERGY`; otherwise spend single minerals from the **largest** pile while loose minerals can cover the remainder (preserves set-balance, no needless set-burn); otherwise — a sub-10 remainder singles can't reach — burn a set, slightly **overshooting** (energy is destroyed, not refunded). `maxEnergy(treasury) = sets*10 + leftoverSingles*1`.

**Resolve phase order (all orders are simultaneous; engine applies in this fixed order):**
1. **Validate & budget.** For each Cog, worst-case spend = `Σ align.energy + (#transfers × TRANSFER_FEE) + bid`. Minerals sent by transfers are reserved first; the remaining treasury must satisfy `maxEnergy(treasury − sent) >= worstCaseSpend`. An illegal or over-budget order set is **rejected whole** (Cog does nothing that turn) and logged.
2. **Auction.** Sealed-bid, second-price, among valid Cogs with `bid > 0`. Highest bid wins the turn's heart; **pays the second-highest bid** (0 if sole bidder — reserve 0). Ties for highest → lowest Cog index (cogOrder order). Winner `hearts += 1`. The clearing price feeds the unified charge below.
3. **Charge.** Each valid Cog's actual spend = `Σ align.energy + #transfers×TRANSFER_FEE + (winner ? clearingPrice : 0)`, charged from `treasury − sent`. Monotonic affordability guarantees success (actual ≤ worst-case). *Charging align energy and the auction payment together is what makes the phase genuinely simultaneous.*
4. **Exploits (before aligns).** For each Exploit on a tile the Cog still owns: windfall `= EXPLOIT_MULT × coherence × density` of the tile's mineral (coherence **before** the drop); then `coherence = 0`, `alignment = null`, `density = floor(density × EXPLOIT_DENSITY)`. Scorched earth happens *before* the scramble.
5. **Align tug-of-war (per targeted tile).** Incumbents reflect post-exploit state. `force(X) = (X === tile.alignment ? tile.coherence : 0) + Σ align.energy by X on this tile`. `winner = argmax force`; `newCoherence = min(COHERENCE_MAX, force(winner) − secondHighestForce)`; `alignment = winner`. **Tie for the max → tile becomes neutral at coherence 0.**
6. **Assemble treasuries.** New treasury = post-charge `+ exploit windfall + incoming transfers`. **Windfall and incoming transfers are next-turn money** (added after the charge, so unspendable this turn). Rejected Cogs keep their treasury but still **receive** incoming transfers and cannot win the auction.
